...
#define TESTING_TAG "TESTING_UART"
...
void uart_console_task(void *pvParameters)
{
    ...
    while(1)
    {
       ESP_LOGI(CONSOLE_UART_TAG,"Hello");
        // read single byte
	...
	int val1 = adc1_get_raw(ADC1_CHANNEL_3);  
     while(1)
  {
     #ifdef CONFIG_MODULE_USEJOYSTICK
      int val1 = adc1_get_raw(ADC1_CHANNEL_3); 
      if(val1>=0&&val1<=1000)
       {
		printf("%d",val1);
		esp_hidd_send_keyboard_value(hid_conn_id,0,ptr,1);
		ESP_LOGI(CONSOLE_UART_TAG,"SPACE Pressed");
		
	}
      else if(val1>=4000&&val1<=4095) 
       {
		printf("%d",val1);
		esp_hidd_send_keyboard_value(hid_conn_id,0,0,1);
		ESP_LOGI(CONSOLE_UART_TAG,"SPACE Pressed");
	} 
	else
       {
		...
		esp_hidd_send_keyboard_value(hid_conn_id,0,0,1);
          ...
          ESP_LOGI(CONSOLE_UART_TAG,"SPACE is not Pressed");
	}
	      printf("\n");                     
...
{
    esp_err_t ret;
ESP_LOGI(CONSOLE_UART_TAG,"Main");
